...
const uint8_t Rcon[13] = {0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36,0x6C,0xD8,0xAB};
...
void mixColumns(uint8_t state[4][4]) {
    uint8_t t[4];
    for (int c = 0; c < 4; ++c) {
        t[0] = gmul(state[0][c], 2) ^ gmul(state[1][c], 3) ^ state[2][c] ^ state[3][c];
        t[1] = state[0][c] ^ gmul(state[1][c], 2) ^ gmul(state[2][c], 3) ^ state[3][c];
        t[2] = state[0][c] ^ state[1][c] ^ gmul(state[2][c], 2) ^ gmul(state[3][c], 3);
        t[3] = gmul(state[0][c], 3) ^ state[1][c] ^ state[2][c] ^ gmul(state[3][c], 2);
        for (int r = 0; r < 4; ++r)
            state[r][c] = t[r];
    }
}
...
// AES-192 key expansion (generates 13 round keys of 16 bytes each)
void keyExpansionAES192(const uint8_t* key, vector<vector<uint8_t>>& roundKeys) {
    int Nk = 6, Nb = 4, Nr = 12;
    roundKeys.resize(Nr + 1, vector<uint8_t>(16));

    vector<uint8_t> expanded(208); // 13 * 16
    for (int i = 0; i < 24; ++i)
        expanded[i] = key[i];

    int bytesGenerated = 24, rconIdx = 0;
    uint8_t temp[4];
    ...
    while (bytesGenerated < 208) {
        ...
        for (int i = 0; i < 4; ++i)
            expanded[bytesGenerated] = expanded[bytesGenerated - 24] ^ temp[i], ++bytesGenerated;
    }
...
string aesEncrypt192(const string& hexInput, const string& hexKey) {
    ...
    for (int round = 1; round < 12; ++round) {
        ...
        shiftRows(state);
        mixColumns(state);
        addRoundKey(state, roundKeys[round].data());
    }
...
int main() {
    ...
    string ciphertext = aesEncrypt192(hexInput, hexKey);
    cout << "AES-192 Ciphertext: " << ciphertext << endl;
