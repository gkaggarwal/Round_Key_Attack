...
string observedLastRoundCiphertext() {
    string input;
    cout << "Enter observed AES last round ciphertext (or press Enter to use default): ";
    getline(cin, input);
    if (input.empty()) {
      //  return "3925841d02dc09fbdc118597196a0b32";
        return "b3779acbaa8d46ea825c6fabd5006fc5"; // Default simulated value
    } else {
        return input;
    }
}
...
vector<vector<string>> readAESCiphertextsFromFile(const string& filename) {
    ...
    while (getline(infile, line)) {
        ...
        if (cycleData.size() == 11) {
            aesCiphertexts.push_back(cycleData);
        } else {
            cerr << "Warning: Each line should contain exactly 11 ciphertexts." << endl;
        }
    }
...
vector<int> identifyRoundCounterScanCells(const vector<vector<string>>& aesCiphertexts) {
    ...
    for (int i = 0; i < delta.size(); ++i) {
        for (int k = 0; k < 4; ++k) {
            if (delta[i] == (10 - static_cast<int>(pow(2, k)))) {
...
string retrieveAESLastRoundKey(const vector<int>& counterScanCells) {
    ...
    string aesLastCipher = observedLastRoundCiphertext();
...
int main() {
    ...
    if (filename.empty()) {
        filename = "state_files/AES-128_ciphertexts_256.txt";
    }
    ...
    std::string originalKey = reverseAESRound10Key(aesRoundKey);
