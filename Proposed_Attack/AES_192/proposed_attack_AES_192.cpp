...
string observedLastRoundFirstCiphertext() {
    string input;
    cout << "Enter observed AES last round ciphertext with scan cells set on 1011(or press Enter to use default): ";
    getline(cin, input);
    if (input.empty()) {
        return "c7f46950791bbf6aa77ba11280c77e3e"; // Default simulated value
    } else {
        return input;
    }
}
...
string observedLastRoundSecondCiphertext() {
    string input;
    cout << "Enter observed AES last round ciphertext with scan cells set on 1010(or press Enter to use default): ";
    getline(cin, input);
    if (input.empty()) {
        return "de091be201718aa6bca53df509df780d"; // Default simulated value
    } else {
        return input;
    }
}
...
vector<vector<string>> readAESCiphertextsFromFile(const string& filename) {
    ...
    while (getline(infile, line)) {
        ...
        if (cycleData.size() == 13) {
            aesCiphertexts.push_back(cycleData);
        } else {
            cerr << "Warning: Each line should contain exactly 13 ciphertexts." << endl;
        }
    }
...
vector<int> identifyRoundCounterScanCells(const vector<vector<string>>& aesCiphertexts) {
    ...
    for (int i = 0; i < delta.size(); ++i) {
        for (int k = 0; k < 4; ++k) {
            if (delta[i] == (12 - static_cast<int>(pow(2, k)))) {
...
int main() {
    ...
    if (filename.empty()) {
        filename = "state_files/AES-192_ciphertexts_256.txt";
    }
    ...
    string Final_key = aesRoundKey + aesSecondLastRoundKey.substr(halfLen);
